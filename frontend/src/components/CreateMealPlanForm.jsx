...
const CreateMealPlanForm = () => {
  ...
  const [MiddleEasternCuisine, setMiddleEasternCuisine] = useState(false)
  ...
  const handleOnChangeMiddleEasternCuisine = () => {
    setMiddleEasternCuisine(!MiddleEasternCuisine);
  };
  ...
  const submitForm = (e) => {
    ...
    const foodPreferences = {
      ...
      cuisine: {
        ...
        asianCuisine: asianCuisine,
        MiddleEasternCuisine: MiddleEasternCuisine,
      },
    }
    console.log("gooing to mealmathcer.jsx")
    navigate("/mealplan");

    // Parse the foodPrefences to Meal Matcher
    // Parse MealMatcher results to the MealPlanPage
    // Parse the foodPreferences to Meal Matcher
    const mealMatcherResults = MealMatcher(foodPreferences);
    console.log(mealMatcherResults)
    console.log("mealmatcherresults")

    // Parse MealMatcher array results to the MealPlanPage
    //load results into prefs parameter of MealPlanPage
    navigate("/mealplan", {state: {prefs: mealMatcherResults}});
  ...
  return (
    ...
    <form onSubmit={submitForm} className="max-w-sm mx-auto pt-20 pb-20">
        ...
        <legend className="sr-only">Cuisines checkboxes</legend>
            ...
            <div className="flex items-center mb-4">
                <input value={MiddleEasternCuisine} onChange={handleOnChangeMiddleEasternCuisine} 
                ...
                <label htmlFor="checkbox-3" className="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Middle Eastern</label>
            </div>
        </fieldset>
